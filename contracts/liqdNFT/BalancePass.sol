...
contract BalancePass is ERC721AQueryable, Ownable {
    ...
    uint256 public maxMintWalletLimit;
    uint256 public whitelist1MintStartTimestamp;
    uint256 public whitelist2MintStartTimestamp;
    ...
    event NftMinted(address indexed _user, uint256 _tokenId);
    ...
    constructor(
        uint256 _maxMint,
        uint256 _maxMintWalletLimit,
        string memory _baseTokenURI,
        uint256 _whitelist1MintStartTimestamp,
        uint256 _whitelist2MintStartTimestamp,
    ...
    function setMaxMintWalletLimit(uint256 _maxMintWalletLimit)
        external
        onlyOwner
    {
        maxMintWalletLimit = _maxMintWalletLimit;
    ...
    function setWhitelist1MintStartTimestamp(
        uint256 _whitelist1MintStartTimestamp
    ) external onlyOwner {
        whitelist1MintStartTimestamp = _whitelist1MintStartTimestamp;
    }
    ...
    function setWhitelist2MintStartTimestamp(
        uint256 _whitelist2MintStartTimestamp
    ) external onlyOwner {
        whitelist2MintStartTimestamp = _whitelist2MintStartTimestamp;
    }
    ...
    function setPublicMintStartTimestamp(uint256 _publicMintStartTimestamp)
        external
        onlyOwner
    {
        publicMintStartTimestamp = _publicMintStartTimestamp;
    ...
    function mint(
        ...
        if (_merkleProof1.length > 0) {
            require(
                block.timestamp >= whitelist1MintStartTimestamp,
                "WHITELIST1_MINT_DIDNT_START"
            );

            // verify against merkle root
            require(
                MerkleProof.verify(_merkleProof1, whitelist1Root, leaf),
                "BalancePass: Invalid proof"
            );
        } else if (_merkleProof2.length > 0) {
            require(
                block.timestamp >= whitelist2MintStartTimestamp,
                "WHITELIST2_MINT_DIDNT_START"
            );

            // verify against merkle root
            require(
                MerkleProof.verify(_merkleProof2, whitelist2Root, leaf),
                "BalancePass: Invalid proof"
            );
        } else {
            require(
    ...
    /// @notice owner mint
    function mint_owner() external payable onlyOwner returns (uint256) {
        return doMint(false);
    }

    function doMint(bool _limitCheck) internal returns (uint256) {
        ...
        if (_limitCheck) {
            require(
                mintWalletLimit[msg.sender] + 1 <= maxMintWalletLimit,
